...
class Finance:
    ...
    def getReplyMsg(self, symbol, name):
        ...
        if df['signal'][-1] == 1:
            keyword = symbol + f'收盤價[{last_close:.2f}元], 已經出現買入訊號了!\n' \
            ...
            keyword = symbol + f'收盤價[{last_close:.2f}元], 已高於MA20月均線及MA60季均線且超過布林上限。\n' \
                    + '強烈建議可賣出。\n' \
            ...
            keyword = symbol + f'收盤價[{last_close:.2f}元], 已高於MA20月均線且超過布林上限。\n' \
                    + '若有賣出計畫，建議可賣出。\n' \
                    + '賣出參考價格為' + str(round(last_close*1.005, 2)) + '元。\n' \
                    + 'RSI指標為 ' + str(round(last_rsi, 2)) 
        elif last_close <= df['MA20'][-1] and last_close <= df['MA60'][-1] and last_close <= df['lower'][-1]*1.005:
            keyword = symbol + f'收盤價[{last_close:.2f}元], 已低於MA20月均線及MA60季均線且低於布林下限。\n' \
                    +'強烈建議可加碼買進!!! (讓您發財~)\n' \
                    +'RSI指標為 ' + str(round(last_rsi, 2))    
        elif last_close <= df['MA20'][-1] and last_close <= df['lower'][-1]*1.005:
            keyword = symbol + f'收盤價[{last_close:.2f}元], 已低於MA20月均線且低於布林下限。\n' \
                    + '若有要加碼買進的計畫，建議可以準備了喔。 (讓您發財~)\n' \
                    + 'RSI指標為 ' + str(round(last_rsi, 2))    
        else:
            keyword = symbol + f'收盤價[{last_close:.2f}元], 還在布林通道內。\n' \
                    + '建議持續觀望~\n' \
                    + 'RSI指標為 ' + str(round(last_rsi, 2))
